...
def process_data(df):
    ...
    # origin airport, destination airport, date, planned time (CRS_DEP), actual time (DEP)
    feature_columns = ["MONTH", "DAY_OF_MONTH", "CRS_DEP_TIME", "DEP_TIME"]

    # these cells of DEP_TIME have NaN values because the flights were cancelled
    ...
    # discretize the scheduled departure time into hour + minute (rounded up to the nearest 10)
    df["CRS_DEP_10MIN"] = df["CRS_DEP_TIME"].apply(
        lambda x: (x // 100) * 100 + round((x % 100) / 10) * 10
    )
    feature_columns.append("CRS_DEP_10MIN")
    feature_columns.remove("CRS_DEP_TIME")
    feature_columns.remove("DEP_TIME")
...
def train_model(df, feature_columns):
    """
    Trains a LightGBM model on the departure delay
...
def generate_time_interval(day, month, interval):
    ...
    :param interval: the time interval (1 or 3 days)
    :returns: a list of moments to predict delays for
    """
    base_date = datetime.datetime(2024, month, day)
    # if interval == 3 days, the start date is 1 day before, end date is 1 day after
    if interval == 3:
...
def predict_delays(model, day, month, carrier, origin_airport, dest_airport, interval):
    """
    Predicts departure delays for selected date, carrier(s),
    ...
    instance_df["DEP_DELAY_NEW"] = y_pred
...
def create_bargraph(
    ...
    merged_predictions["DATE_COMBINED"] = pd.to_datetime(
        merged_predictions.assign(
            year=2024,
            hour=merged_predictions["CRS_DEP_10MIN"] // 100,
            minute=round((merged_predictions["CRS_DEP_10MIN"] % 100) / 10) * 10,
